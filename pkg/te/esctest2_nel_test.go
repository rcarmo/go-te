...
// From esctest2/esctest/tests/nel.py::test_NEL_MovesDoesNotScrollOutsideLeftRight
func TestEsctestNelTestNELMovesDoesNotScrollOutsideLeftRight(t *testing.T) {
	screen := NewScreen(80, 24)
	stream := NewStream(screen, false)
	...
	esctestNEL(t, stream)
	esctestAssertEQ(t, esctestGetCursorPosition(screen), esctestPoint{X: 2, Y: height})
	esctestAssertScreenCharsInRectEqual(t, screen, esctestRect{Left: 3, Top: 5, Right: 3, Bottom: 5}, []string{"x"})
